...
function VS_Setup_1719 (src, version)  // Почти 25/17
    {
...
function VS_CopyFiles_1719 (dest, src, version)
    ...
    src   = FS.GetAbsolutePathName (src + "\\..\\..\\..\\.."); // Debug ("  ...FS.GetAbsolutePathName() = '" + src + "'");
    src   = ConvertCP (src, "Windows-1251", "UTF-8");          // Debug ("  ...ConvertCP() = '"              + src + "'");
    dest += "\\";
    ...
    try
        ...
        if (replace != null)
            ...
            for (var i = 0; i < replace.length; i += 2)
                {
                Debug ("  ...Replace (" + replace[i] + ", " + replace[i+1] + ")");
...
function ConvertCP (text, sourceCP, destCP)
    ...
    if (destCP == "UTF-8") text = text.substr (3, text.length-3);  // Skip the damned BOM
