...
void OnscreenHelp (int level, bool error = false);
...
int main()
    ...
    for (;;)
        ...
        DrawBackground();
        OnscreenHelp (g.graph.size_, (g.graph.size_ >= Graph_t::MaxSize-1));
...
void OnscreenHelp (int level, bool error)
    {
    ...
    if (error)
        {
        txSetColor (TX_PINK);
        txTextOut (c.x, c.y + 50, "Graph maximum capacity is reached!");
        }
    }
